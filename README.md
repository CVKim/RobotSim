...
| 인식→제어 폐루프 (트윈, 60회) | mocap 석션: oracle **100%** vs 인식 구동 43.3% → **층 선택 시간 사전 78.3%** (인식 비용 56.7 → 21.7%p, 11.9 s/픽) · UR10e+트랙: oracle **100%** vs 인식 65 → **75%** (7.8 s) · **ROS2 그래프가 트윈의 팔을 구동**: 12박스 중 12개 배치, 실패 0 | [상세 §9](docs/30_결과_상세.md#9-인식--제어-폐루프-셀-트윈에서-실제로-집어-옮기기) · §10 · §11 · [docs/41 8절](docs/41_셀_트윈.md) · `results/twin_closed_loop*.json` · `results/ros2_twin_cycle.json` |
...
| 경로 계획 (장애물 회피) | 관절 공간 RRT-Connect + 단축: 같은 이동 48건에서 직선 71% → **100%**, 계획 85 ms, 돌아가는 값 +0.25 s. 같은 문제를 **MoveIt2(OMPL)** 로도 계획해 비교(11 ms, 경로 3% 짧음) | [상세 §14](docs/30_결과_상세.md#14-장애물을-돌아가는-경로-계획--직접-구현과-moveit2-비교) · `results/twin_path_plan.json` |
| 일반화 한계 (트윈) | 혼합 SKU 4종에서 리콜 80 → **22%** (원인은 SKU 사전이 아니라 층 선택 — 넓혀도 그대로, 후보 깊이 병합 시 48%) · 카메라 기울기는 **5도까지** 유지되고 10도에서 절반 | [상세 §13](docs/30_결과_상세.md#13-일반화-한계--단일-sku탑다운-가정이-어디서-깨지는가) · `results/twin_generalization.json` |
...
| 테스트 | pytest **104** (98개는 합성 프레임만으로) · 30세션 전체 파리티 v1/v2 · 대차 4세션 파리티 · URDF↔MJCF 기구학 대조 · colcon test 39 (launch_testing 통합 5 · bag 재생) | `tests/` · `ros2_ws/.../test/` |
...
![dock demo](assets/dock_demo.gif)

*화면 설명: 도킹 한 회를 녹화한 것이다. 고정 프레임이 대차라 대차가 멈춰 있고 카메라(AGV)가 결합 위치로 다가온다.
판이 데크, 자홍 선이 앞 테두리, 초록 기둥이 고리이며 라벨의 숫자가 대차 좌표에서 본 고리 위치다.*
...
인식 노드도 같은 트윈을 카메라로 쓰므로 로봇이 옮긴 결과가 다음 프레임에 그대로 보인다. 12박스 팔레트를 명령 12회에 12개 배치, 실패 0 으로 비웠다
(층 선택 시간 사전 켬; 사전 없이는 10개에서 층 선택이 놓쳐 멈췄다). 첫 실행에서는 12개 중 6개가 놓는 순간 튕겨 나갔고, 그 추적이 픽 포즈 요 규약·페이로드 충돌 검사·
목적지 슬롯 겹침 세 결함을 드러냈다([docs/41 7절](docs/41_셀_트윈.md)).

![twin cycle](assets/twin_cycle_demo.gif)

*화면 설명: 12박스 사이클을 처음부터 끝까지 녹화한 것이다(헤드리스 rviz, 1초 간격 95프레임). 파란 팔이 트윈의 UR10e 로,
트윈이 보낸 관절각을 `/joint_states` 로 받아 URDF 로 그린 것이다.
...
- 단일 SKU · 단일 카메라 포즈 · 연속 시퀀스 30프레임(레이아웃 2종). 다른 조명·손상 박스는 미검증이고, 혼합 SKU 와 카메라 기울기는 실측 대신 **트윈에서 한계를 쟀다**(§13)
- 실측 정답은 검출기 출력 + RGB 육안 대조(pseudo-GT). 절대 오차는 트윈에서만 측정
- 폐루프의 팔 실행기는 IK·충돌 검사·관절 속도까지 본다. 장애물을 돌아가는 경로 계획은 §14 에서 붙였고(직선 71% → 100%), 이 셀의 폐루프 성적은 바뀌지 않았다(실행기가 이미 IK 해를 바꿔 피한다). 석션 물리는 없음. 절대값이 아니라 **oracle 대비 인식 비용**이 결과
...
python -m pytest tests -q                                          # 104 passed (실측 데이터가 없으면 6건 skip, mujoco 가 없으면 팔·URDF 9건 skip)
