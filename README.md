...
깊이 히스토그램 층 검출과 강도(I) 채널 에지 분리로 박스를 검출한다 (라벨·학습 없음). v1 은 30세션에서 148개를 검출했고,
...
- **v1 누락 19개의 원인**: 2층 4×3 격자 중앙 2개가 강도 에지 절단 실패로 병합(7프레임 13박스), 층 말미 잔여 박스 1~2개가
  중앙 ROI 히스토그램 피크의 바닥/티어시트 전환으로 누락(4프레임 6박스)
...
|---|---|---|
| 지연시간 (CPU, 층 검출~치수 전 단계) | **52 ms/프레임** | 226 ms/프레임 |
| 클린 프레임 | 148/167 = 88.6% | **167/167**, 빈 프레임 오검출 0 |
...
이전 표의 v1 수치(100%/91%/72%)는 v1 자체의 클린 출력 148개를 기준으로 한 값이었고, 검증된 실박스 167개 기준으로 다시 계산했다.
