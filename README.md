...
| AI 비전 ↔ 로봇 제어 연동 | 셀 트윈에서 **인식 출력만으로 집어 옮기는 폐루프** 성공률 46.7% (정답 포즈 oracle 100%, 사이클 11.7초/픽) | [tools/twin_closed_loop.py](tools/twin_closed_loop.py) |
...
| perception (ToF 검출) | **46.7%** (28/60) | 소스 미검출 6, 흡착 실패 5, 전 후보 거부 3 |

**인식이 잃는 성능: 53.3%p.** 지배적 실패는 '소스 미검출'(층에 박스가 남았는데 검출 0 → 조기 종료)로,
...
다만 셀 트윈에서 측정한 사이클은 11.7초/픽이라 인식 지연은 사이클의 3% 로 병목이 아니다.
