...
기하 인식 파이프라인을 실제 배포 관점에서 점검한 결과:

| 항목 | 측정값 |
|---|---|
| 지연시간 | **55 ms/프레임** (CPU, 층 검출~치수~픽포인트 전 단계) |
| 추가 무효 픽셀 30% | 검출 100% 유지, 치수 -3% 바이어스 |
| 깊이 노이즈 σ 20mm | 검출 91% |
| 강도 대비 0.1× | 검출 100% (에지 분리가 스케일 불변) |
| 대면적 결손 15 / 30개 | 검출 86% / **72%** — 가장 큰 약점 |

![robustness](assets/robustness_degradation.png)

**한계**: 단일 SKU·단일 카메라 포즈·30프레임에서만 검증됨. sim2real의 0.99는 정답 라벨이 자체 검출기 출력이고
실측 6장이 동일 장비에서 나온 것이라 낙관적이며, 합성 전용 성능(0.43~0.56)이 보수적 수치다.
