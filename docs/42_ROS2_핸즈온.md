...
클라이언트는 제어·도킹 노드), **액션**(`/robot/execute_pick` — 서버는 트윈 브리지, 6-e), **커스텀 인터페이스**(`robotsim_interfaces`:
`ExecutePick.action`, `DockState.msg`), **파라미터**(`source`, `rate_hz`, `min_confidence`, `temporal_prior` … + YAML 파일),
**TF**(카메라↔로봇 정적, 카메라↔대차 동적, 그리고 그것을 **쓰는** `lookup_transform`), **URDF + robot_state_publisher**(rviz 의 UR10e),
**QoS**(포인트클라우드는 best-effort, 정적 TF 는 transient_local), **launch**(노드 + rviz2 동시 기동),
**패키지 빌드**(ament_python·ament_cmake, colcon), **통합 테스트**(launch_testing).
...
세 층이다 (colcon test 39건 + 트윈 서버가 있을 때만 도는 통합 테스트 1건 = 40):
...
- **통합(launch_testing)** `pick_executor/test/test_pick_cycle_launch.py` · `test_bag_replay_launch.py` · `test_twin_cycle_launch.py`(6-d) · `test_action_cycle_launch.py`(6-e: 가짜 액션 서버로 사이클) · `robotsim_perception_ros/test/test_dock_launch.py`(8-c: 결합 + 좌표 변환 사슬):
  ...
  WSL2 에서는 노드끼리 UDP 로만 통신하도록 테스트 파일이 Fast DDS 프로필과 전용 `ROS_DOMAIN_ID`(71~79)를 환경에 넣는다 —
...
powershell -ExecutionPolicy Bypass -File scripts	win_cycle_demo.ps1 -UseAction true -Gif /mnt/e/Robot_Sim/assets/twin_cycle_demo.gif   # 액션 경로 + 화면 GIF 녹화
...
*rviz 화면(세 번째 명령 실행 중): 오른쪽의 파란 팔이 트윈의 UR10e 다 — 트윈이 보낸 관절각을 `/joint_states` 로 받아 URDF 로 그렸고,
회색 원기둥이 받침대, 그 위 짧은 원통이 리니어 트랙 캐리지다. 초록·주황 판 = 인식 노드가 본 남은 박스(직접 검출 / 격자 보완),
노란 세 점 = 이번 명령의 pre-pick·pick·lift, 하늘색 선 = 팔 끝(TCP)이 지나온 경로(소스 → 목적지 → 후퇴), 작은 축들이 링크 좌표계와 `base_link` 다.
ToF 점구름은 켜져 있지만 이 시점 화면에서는 박스 판과 팔 모델에 가려 거의 보이지 않는다. 카메라에 팔이 보인다는 사실 자체가 층 선택에
영향을 준다(docs/41 7절 C).*
...
### 6-e. 계약을 타입으로 — 커스텀 인터페이스와 액션, 그리고 rviz 에 서는 팔

여기까지의 노드는 명령을 `geometry_msgs/PoseArray` 로, 결과와 상태를 `std_msgs/String` 안의 JSON 으로 주고받았다. 돌아가기는 하지만
계약이 주석에만 있어서 필드 이름이 틀려도 빌드가 통과하고 런타임에 조용히 비어 버린다. 그래서 패키지 하나를 더 만들었다 —
`robotsim_interfaces` (`ament_cmake` + `rosidl_default_generators`; 인터페이스는 순수 파이썬 패키지로 만들 수 없다).

| 무엇 | 어디에 | 왜 그 형태인가 |
|---|---|---|
| `action/ExecutePick.action` | `pick_executor`(클라이언트) ↔ `twin_bridge`(서버) | 픽 하나가 수 초~수십 초다. 목표 하나에 결과 하나가 붙고, 진행 중 피드백이 오며, 실행 중 새 목표는 서버가 **거절**한다 |
| `msg/DockState.msg` | `dock_node` → `/dock/state` | 도킹 한 프레임의 상태. 필드 단위로 echo·bag 재생·플로팅이 된다 |

액션으로 돌리기: `use_action:=true` 를 주면 pick_executor 가 `/robot/target_poses` 대신 목표를 보내고 결과를 액션으로 받는다.
같은 12박스 사이클이 그대로 돈다(아래 기록).

```bash
ros2 launch twin_bridge twin_cycle.launch.py use_action:=true
ros2 action list -t                     # /robot/execute_pick [robotsim_interfaces/action/ExecutePick]
ros2 topic echo /dock/state             # DockState (mode, e_u_mm, e_v_mm, e_yaw_deg, docked, tf_check_mm)
```

구현에서 걸리는 곳 둘.
1. **액션 실행 콜백은 오래 블로킹한다.** 단일 스레드 spin 이면 그동안 상태 타이머·`~/reset` 서비스가 멈춘다 →
   `ReentrantCallbackGroup` + `MultiThreadedExecutor` 로 돌린다.
2. **취소를 받지 않는다(REJECT).** 트윈의 한 실행은 TCP 요청 하나로 끝까지 도는 구조라 중간에 끊을 수단이 없다. 실제 드라이버라면
   여기서 정지 명령을 보낸다 — 못 하는 것을 인터페이스에서 할 수 있는 척하지 않는 편이 낫다.

**rviz 에 팔이 선다.** 트윈은 MJCF 로 팔을 돌리고 rviz 는 URDF 만 안다. 두 파일에 같은 기구학을 손으로 두 번 적으면 반드시 어긋나므로
`tools/make_ur10e_urdf.py` 가 MJCF(`sim/assets/ur10e/ur10e.xml`)에서 바디 위치·쿼터니언·관절 축·한계를 읽어 URDF 를 만든다. 받침대·트랙
배치도 `sim/arm.py` 와 같은 값을 쓴다. `twin_bridge` 가 트윈의 관절각을 `/joint_states` 로 내고 `robot_state_publisher` 가 링크 TF 를 낸다.

두 기구학이 같은지는 눈이 아니라 테스트가 본다: `tests/test_urdf_fk.py` 가 무작위 관절각 12개(트랙 포함)에서 URDF 순기구학과 MuJoCo
순기구학의 TCP 를 비교한다 — 차이 0.1 mm 미만.

```bash
.venv\Scripts\python.exe tools/make_ur10e_urdf.py      # ros2_ws/src/twin_bridge/urdf/ur10e_track.urdf
python -m pytest tests/test_urdf_fk.py -q
```

**파라미터를 파일로.** 도킹 쪽 값들은 `robotsim_perception_ros/config/dock_params.yaml` 에 모았고 launch 가 그것을 읽는다
(`params_file:=` 로 통째로 갈아끼울 수 있다). 돌고 있는 노드의 실제 값은 `ros2 param dump /dock_node` 로 확인한다.

**TF 를 쓰는 쪽도 하나 둔다.** TF 를 내기만 하고 아무도 안 쓰면 규약이 틀려도 조용히 넘어간다. `dock_node` 는 매 프레임
`/cart/hook_pose`(카메라 좌표)를 `lookup_transform(cart ← tof_optical)` 로 옮겨 `cart_node` 가 낸 대차 좌표와 비교하고 그 차이를
`DockState.tf_check_mm` 로 낸다. 실행 중 값은 0.01 mm 아래이고, 통합 테스트가 1 mm 를 넘으면 실패한다.
...
### 6-f. MoveIt2 — 같은 계획 문제를 표준 플래너로도 풀어 본다

`robotsim_moveit_config` 에 SRDF·kinematics·joint_limits·OMPL 설정을 두고 `move_group` 을 **계획 전용**으로 띄운다
(실행 컨트롤러는 붙이지 않는다 — 궤적을 실제로 도는 것은 MuJoCo 트윈이다). 로봇 모델은 6-e 의 URDF 를 그대로 쓴다.

```bash
# (Windows) 같은 문제와 장면을 뽑아 둔다
.venv\Scripts\python.exe tools	win_path_plan.py --seeds 4 --dump-problems explore	win\plan_problems.json
# (WSL2) move_group 을 띄우고 48건을 계획해 비교
bash scripts/wsl_moveit_compare.sh
ros2 launch robotsim_moveit_config move_group.launch.py     # rviz MotionPlanning 으로 직접 만져 볼 때
```

결과(48건): 우리 구현(파이썬 RRT-Connect) 48/48·계획 85 ms, MoveIt2(C++ OMPL) 48/48·계획 11 ms, 경로 길이 평균 10.24 vs 10.61 rad.
직선이 막힌 14건은 양쪽 모두 풀었다. 수치는 `results/twin_path_plan.json` 의 `moveit` 항목.

두 가지가 걸렸다.
1. **설정 YAML 을 경로로 넘기면 안 된다.** `move_group` 은 노드 파라미터 파일 형식을 기대하므로 그냥 주면
   `Couldn't parse params file` 로 죽는다. launch 에서 `yaml.safe_load` 로 읽어 dict 로 넘긴다.
2. **URDF 에 있는 것을 계획 장면에 또 올리면 안 된다.** 받침대·레일을 장면에도 넣었더니 로봇이 '자기 받침대와 충돌'
   상태로 시작해 MoveIt 이 시작 자세를 빼내며 돌아갔다(경로 길이 3 → 13 rad). 트윈 도형을 내보낼 때 그 셋을 뺀다.
...
화면 녹화: 두 데모 모두 마지막 인자로 GIF 경로를 주면 헤드리스 rviz 를 캡처해 GIF 로 만든다(`scripts/lib_record.sh`, ImageMagick). ffmpeg 없이 도는 것이 목적이라 `import` 로 잡아 `convert` 로 합치고,
검은 스플래시 프레임은 파일 크기로 걸러 내며 프레임이 많으면 고르게 솎는다.
...
*rviz(고정 프레임 = 대차): 파란 판이 데크, 자홍 선이 앞 테두리(림), 노란 선이 레일, 초록 기둥이 고리이고 이 마커들은 대차 프레임에 고정돼 있다.
오른쪽 위 `tof_optical` 축이 AGV 에 실린 카메라, 가운데 아래 `cart` 축이 매 프레임 다시 추정한 대차 좌표계다. 라벨은 이 프레임의 고리 위치
(u 36, v −1, h 233 mm). 한 장면이라 접근 자체는 보이지 않는다 — 도킹이 진행되면 카메라 축이 고리 앞 결합 위치 쪽으로 옮겨 간다.
경과는 `/dock/state`(DockState)로 보고 결과 수치는 위 표에 있다.*

```bash
bash scripts/wsl_dock_demo.sh - - /mnt/e/Robot_Sim/assets/dock_demo.gif   # 헤드리스 rviz 를 GIF 로 녹화
