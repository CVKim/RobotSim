...
- **통합(launch_testing)** `pick_executor/test/test_pick_cycle_launch.py` · `test_bag_replay_launch.py` · `test_twin_cycle_launch.py`(6-d) · `robotsim_perception_ros/test/test_dock_launch.py`(8-b):
...
판단 로직은 `pick_executor/logic.py` 에 ROS 없이 순수 파이썬으로 있고(`test/test_logic.py` 13건, Windows 의 pytest 로도 돈다),
...
3. **twin_bridge** — `/robot/target_poses` 를 받아 트윈에 실행을 요청하고(작업 스레드; 노드는 계속 spin), 돌아온 TCP 경로를 시뮬 시각대로
   ...
   트윈은 시뮬 8 s 를 1.2 s 에 계산하므로 재생 없이는 사이클이 실제보다 빨리 돈다. 결과에는 `pick_err_mm`(명령 픽 위치 vs 실제로 집은 박스
...
| 명령 픽 위치 vs 집은 박스 중심 (pick_err) | 중앙값 7 mm, 최대 89 mm | 인식 오차와 노드↔트윈 좌표 불일치가 합쳐진 값. 89 mm 는 격자 보완으로 채운 칸(직접 검출 아님)을 집은 경우 |
| 시뮬 사이클 | 평균 7.5 s (6.3~10.9) | 관절 속도 한계·직선 하강 0.4배 속도·흡착/해제 대기 포함 (docs/41 7절 C 의 7.9 s 와 같은 정의) |
| 계산 시간 | 실행 1회 1.2 s, 렌더+인식 0.5 s | 트윈이 실시간보다 6배 빠르다 → 브리지가 재생으로 실시간에 맞춘다 |
...
- 쓸 수 있음: WSL2 Linux 에 ROS2 Humble 환경 구축, ament_python 패키지 빌드, 노드·토픽·서비스·파라미터·TF·QoS·launch 사용,
  ...
  인식 출력(고리 포즈)으로 AGV 를 결합 위치까지 몰고 가는 도킹 폐루프 — 제어기·운동학 플랜트·stop-and-go 촬영 동기화(8-b),
...
### 8-b. 대차 도킹 폐루프 — `dock_node` + `agv_sim_node`
...
| 인식 구동 | **27/30** | 9.1 s | 측방 3.0 mm · 거리 6.8 mm · 요 1.23° (p95 측방 6.3·거리 9.7 mm) |

결합 판정은 측정 기준(|u| < 10, |v − 200| < 10 mm, |요| < 2°)이고 표의 오차는 정답 기준이라 인식 오차가 더해진 값이다. 고리 인식의 위치 오차가
...
*왼쪽: 인식 구동 30회의 궤적(고리 위치를 AGV 기준으로 그림; 별 = 결합 위치). 가운데: 결합 판정 시점의 실제 오차, 점선이 판정 허용치. 오른쪽: 도킹 시간.*
