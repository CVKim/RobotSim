...
세 층이다 (colcon test 37건, 트윈 서버가 있을 때만 도는 통합 테스트 1건은 기본 건너뜀):
...
- **통합(launch_testing)** `pick_executor/test/test_pick_cycle_launch.py` · `test_bag_replay_launch.py` · `test_twin_cycle_launch.py`(6-d):
...
**기록 (12박스, UR10e + 트랙, 인식 노이즈 모델 켬)**

| 항목 | 값 | 뜻 |
|---|---|---|
| 프레임 / 명령 | 13 / 10 | 프레임마다 인식 → 첫 후보를 명령. 마지막 3프레임은 LAYER_EMPTY |
| 배치 / 실패 | 10 / 0 | 트윈의 팔이 명령 10회를 전부 집어 목적지 3×3 격자에 놓았다 (놓은 위치 오차 0.3~5 mm) |
| 남은 박스 | 2 | 박스 2개가 남자 층 선택이 데크를 '상면'으로 골라 검출 0 → 3회 연속으로 종료. docs/41 5절의 잔여 ≤3개 실패 모드 |
| 프레임별 검출/정답 | 4/12, 4/11, 10/10, 4/9, 7/8, 6/7, 6/6, 5/5, 4/4, 3/3 | 박스가 많을 때 격자 보완이 못 채운 칸이 많고(첫 프레임 12 중 4), 6개 이하에서는 전부 찾는다 |
