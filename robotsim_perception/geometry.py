# -*- coding: utf-8 -*-
"""ToF 상면 박스 검출 기하 알고리즘 — v1(detect_boxes) + v2(detect_boxes_v2).

**단일 원본(single source of truth).** 이전에는 이 알고리즘이 tools/binpick_topface.py 와
robotsim_perception/detect.py 에 각각 복제돼 있어, 센티넬 오염 버그를 두 곳에서 따로 고쳐야 했고
파리티 테스트가 세션 1개만 검사해 실제 불일치를 놓쳤다. 이제 여기 한 곳만 둔다.
  - tools/binpick_topface.py : 이 모듈을 재수출 + 시각화(render_overlay*)
  - robotsim_perception.detect : Frame/Box 데이터클래스로 감싼 공개 API

입력 `sess` 는 X/Y/D/I (float32 (H,W), mm) 를 담은 딕셔너리형이면 된다
(실측 .mim 세션, 셀 트윈 렌더, 합성 프레임 모두 동일 형식).

원리 (딥러닝 없이 기하만):
  1. D 히스토그램에서 최상층(카메라에 가장 가까운) 평면 깊이 피크 탐색
  2. 피크 +-tol 픽셀 -> 상면 마스크 -> 모폴로지 정리 -> 연결요소 = 박스 후보
  3. 각 박스: X/Y 좌표맵(mm)에서 min-area rect -> 상면 L x W (mm)
v2 는 위에 박스별 신뢰도와 격자 기반 결손 보완('inferred'), 층 선택 규칙을 더한다.
"""
import cv2
import numpy as np
...
def valid_mask(sess):
    """D 채널 기준 유효 픽셀 마스크 (센티넬·0·비유한값 제외)."""
    D = sess["D"]
    m = np.isfinite(D) & (D > 0) & (D < SENTINEL_D)
    for k in ("X", "Y"):
        if k in sess and sess[k] is not None:
            v = sess[k]
            m &= np.isfinite(v) & (np.abs(v) < SENTINEL_XY)
    return m
