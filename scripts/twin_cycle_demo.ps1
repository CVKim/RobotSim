...
param(
    ...
    [string]$Gif = ""                 # 경로를 주면 rviz 화면을 GIF 로 녹화 (WSL2 ImageMagick)
)
...
try {
    ...
    wsl -d Ubuntu-22.04 -e bash /mnt/e/Robot_Sim/scripts/wsl_twin_cycle_demo.sh "-" "/mnt/e/Robot_Sim/assets/ros2_twin_cycle_rviz.png" "/mnt/e/Robot_Sim/explore/ros2/twin_cycle.log" $MaxWaitS $UseAction $(if ($Gif) { $Gif } else { "-" })
}
finally {
