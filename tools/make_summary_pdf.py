...
def page1(pdf):
    ...
    text(fig, rx, 0.815,
         ...
         "4. 자동 테스트 51건(합성 데이터만으로 46건)과 ROS2 테스트 12건이 돈다.",
         8.2, wrap=0.34)
    ...
    rows = [
        ...
        ("팔레타이징 강화학습 (3시드)",
         ...
         "즉 성능의 핵심은 마스크다."),
        ("ROS2 노드 2개 (Humble, WSL2)",
         "빈피킹 노드, 대차 노드, 테스트 12건 통과",
         "인식 결과를 로봇 제어기가 받는 표준 형식(포즈, 좌표 변환 TF, 마커)으로 내보낸다. 빈피킹은 카메라가 고정이라 "
         "TF가 한 번만 나가고, 대차는 매 프레임 대차 좌표계를 새로 잡아 TF를 계속 갱신한다."),
    ]
    y = 0.385
    for k, v, why in rows:
...
def page2(pdf):
    ...
    H = 0.27
    image(fig, A / "ros2_rviz_synthetic.png", (X[0], 0.45, W, H),
          "4. ROS2 빈피킹 노드를 rviz로 본 화면",
          "인식 결과를 ROS2 메시지로 내보내 rviz에서 본 것이다. 회색 점은 ToF 점구름, 초록 판은 직접 검출한 박스 윗면, 주황 판은 격자 규칙으로 "
          "채운 칸, 판 위의 작은 축은 로봇이 집을 위치와 자세(로봇 기준 좌표), 파란 화살표는 다음에 집을 박스로 들어가는 방향이다. "
          "합성 장면이라 회사 데이터 없이 실행된다.")
