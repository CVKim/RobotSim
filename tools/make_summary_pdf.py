...
def page1(pdf):
    ...
    rows = [
        ...
        ("인식 결과로 집어 옮기기 (시뮬레이터 60회)",
         "mocap 100% vs 46.7%, UR10e 팔 85~88% vs 60%",
         "정답 위치와 인식 결과의 차이(53 %p, 팔은 25~28 %p)가 인식 오류의 비용이다. 손실 대부분은 박스가 1~3개 남았을 때 층을 잘못 골라 "
         "남은 박스를 포기한 것. 팔이 정답에서도 12~15% 잃는 것은 못 닿는 박스와 놓다가 튕기는 시뮬 아티팩트."),
        ("팔 도달 범위 (UR10e, 실측 픽 포즈 167개)",
         "고정 받침대 87%, 리니어 트랙을 달면 100%, 무충돌 96~98%",
...
def page2(pdf):
    ...
    image(fig, A / "twin_closed_loop.png", (X[2], 0.905, W, H),
          ...
          "실제로 움직이는 UR10e 팔(오른쪽 두 막대)은 정답 위치에서도 못 닿는 박스와 놓다가 튕기는 박스로 12~15%를 잃는다. "
          "인식 비용은 세 실행기 모두 박스가 1~3개 남았을 때의 층 선택 실패가 만든다. 가운데: 인식 구동 실패 종류. 오른쪽: 사이클 시간.")
    H = 0.27
