...
def page1(pdf):
    ...
    text(fig, rx, 0.815,
         ...
         "4. 자동 테스트 65건(합성 데이터만으로 46건)과 ROS2 테스트 37건(노드를 실제로 띄우는 통합 테스트 포함)이 돈다.",
         8.2, wrap=0.34)
    ...
    rows = [
        ...
        ("인식 결과로 집어 옮기기 (시뮬레이터 60회)",
         ...
         "손실 대부분은 박스가 1~3개 남았을 때 층을 잘못 골라 남은 박스를 포기한 것이었고, 직전 프레임의 층 깊이를 다음 프레임의 후보로 쓰면 "
         "그 손실이 대부분 사라진다(인식 비용 57 → 22 %p). 트랙 달린 팔은 정답을 주면 60개를 다 놓고, ROS2 노드가 구동하면 12개 중 12개."),
