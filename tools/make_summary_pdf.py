...
def page1(pdf):
    ...
    stages = [
        ...
        ("③ 로봇 좌표", ["평면 피팅 → 중심·법선", "카메라→로봇 4×4 변환(핸드아이)", "6-DoF 픽 포즈(요 포함)"], "중심 오차 8.5 mm"),
        ("④ 셀 트윈 폐루프", ["MuJoCo 로 셀 재현", "인식 결과만으로 집기", "UR10e IK·충돌·경로 계획"], "인식 구동 78%"),
    ...
    tiles = [
        ...
        ("104 + 40", "자동 테스트 (pytest+ROS)", "회사 데이터 없이 98건이 돈다"),
    ]
    ...
    lim = [
        ...
        "크기가 섞이면 검출이 80 → 22%로 떨어진다(층 선택이 원인). 카메라 기울기는 5도까지 버틴다.",
        "실측 정답은 검출 결과를 사람이 눈으로 확인한 것이다. 줄자로 잰 정답은 없다.",
        "팔·AGV 는 기구학 시뮬이다. 흡착·바퀴 물리는 없다(경로 계획은 §14 에서 붙였다).",
        "실제 로봇·센서 드라이버는 없고, 핸드아이 외참도 실측 보드로 한 번 돌리는 것이 남았다.",
    ]
