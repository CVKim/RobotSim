...
def page1(pdf):
    ...
    text(fig, rx, 0.815,
         ...
         "4. 자동 테스트 61건(합성 데이터만으로 46건)과 ROS2 테스트 37건(노드를 실제로 띄우는 통합 테스트 포함)이 돈다.",
         8.2, wrap=0.34)
    ...
    rows = [
        ...
        ("인식 결과로 집어 옮기기 (시뮬레이터 60회)",
         "mocap 100% vs 43.3%, UR10e 트랙 100% vs 65%",
         "정답과 인식의 차이(57 %p, 팔은 35 %p)가 인식 오류의 비용이다. 손실 대부분은 박스가 1~3개 남았을 때 층을 잘못 골라 남은 박스를 "
         "포기한 것. 트랙 달린 팔은 정답을 주면 60개를 다 놓고, ROS2 노드가 구동하면 12개 중 10개."),
        ...
        ("팔레타이징 강화학습 (3시드)",
         ...
         "즉 성능의 핵심은 마스크다."),
        ("ROS2 노드 4개 (Humble, WSL2)",
         "빈피킹, 대차, 제어, 트윈 브리지 노드. 테스트 37건 통과",
         "인식 결과를 로봇 제어기가 받는 표준 형식(포즈, 좌표 변환 TF, 마커)으로 내보낸다. 제어 노드는 픽 포즈를 세 점 궤적으로 "
         "바꿔 보내고 완료 보고를 기다린 뒤 재촬영을 요청한다. 브리지 노드가 그 명령을 시뮬레이터의 팔에 실행시켜 12박스 중 10개를 놓았다(실패 0)."),
    ]
    y = 0.385
...
def page2(pdf):
    ...
    image(fig, A / "twin_closed_loop.png", (X[2], 0.905, W, H),
          "3. 인식 결과만으로 집어 옮기기 (실행기 4종)",
          "시뮬레이터 셀에서 박스를 집어 옆 팔레트로 옮긴다. 왼쪽: 정답 위치를 줄 때(파랑)와 인식 결과로 할 때(주황)의 성공률. "
          "트랙 달린 UR10e(맨 오른쪽)는 정답 위치를 주면 60개를 다 놓고, 고정 받침대는 못 닿는 박스 6개를 잃는다. "
          "인식 비용은 네 실행기 모두 박스가 1~3개 남았을 때의 층 선택 실패가 만든다. 가운데: 인식 구동 실패 종류. 오른쪽: 사이클 시간.")
    H = 0.27
    image(fig, A / "ros2_twin_cycle_rviz.png", (X[0], 0.45, W, H),
          ...
          "찍는다. 12박스 중 10개를 놓고 남은 2개는 층 선택이 놓쳐 멈췄다.")
