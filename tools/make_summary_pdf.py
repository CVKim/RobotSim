...
def page1(pdf):
    ...
    text(fig, rx, 0.815,
         ...
         "4. 자동 테스트 84건(회사 데이터 없이 도는 것 78건)과 ROS2 테스트 39건(노드를 실제로 띄우는 통합 테스트 5건 포함)이 돈다.",
         8.2, wrap=0.34)
    ...
    rows = [
        ...
        ("대차 고리 위치 (실측 4회 촬영)",
         "고리 반복 3.85 mm (면내 3.1 mm) · 도킹(합성 대차) 30회 중 27회 결합",
         "촬영마다 카메라 높이가 6 cm, 대차 위치가 5 cm까지 달라도 데크 평면·테두리·레일로 좌표를 잡으면 고리는 같은 자리다. "
         "이 좌표로 합성 대차에서 AGV(시뮬)를 결합 위치까지 몰았다: 평균 9.1초, 측방 3.0 mm(정답 기준 26회)."),
        ("팔레타이징 강화학습 (3시드)",
         "학습 정책 64.7개 vs 규칙 56.6개 (14% 더 쌓음)",
         "같은 팔레트에 박스를 몇 개 쌓느냐. 놓을 수 없는 자리를 미리 막는 마스크를 빼면 43.2개로 규칙보다 못하다. "
         "즉 성능의 핵심은 마스크다."),
        ("ROS2 노드 6개 (Humble, WSL2)",
         "빈피킹·대차·제어·트윈 브리지·도킹·AGV 시뮬 노드. 액션·커스텀 메시지. 테스트 39건",
...
def page2(pdf):
    ...
    image(fig, A / "twin_closed_loop.png", (X[2], 0.905, W, H),
          ...
          "가운데 그림에서 '흡착 실패'가 6 → 2회, '소스 미검출'이 6 → 2회로 준다. 팔에 정답 위치를 주면 트랙 100%, 고정 받침대 88%. 오른쪽: 사이클 시간.")
    H = 0.27
    image(fig, A / "ros2_twin_cycle_rviz.png", (X[0], 0.45, W, H),
          "4. ROS2 노드가 시뮬레이터의 팔을 구동하는 화면",
          "세 번째 픽이 실행되는 중. 오른쪽 파란 팔이 시뮬레이터의 UR10e 로, 시뮬레이터가 보낸 관절각을 받아 rviz 가 그린 것이다. "
          "회색 원기둥은 팔 받침대, 초록·주황 판은 인식 노드가 본 남은 박스, 노란 세 점이 이번 명령(pre-pick, pick, lift), "
          "하늘색 선이 팔 끝이 지나온 경로다. 팔이 명령을 끝내면 완료를 보고하고, 인식 노드가 다음 프레임을 찍는다. 12박스를 전부 놓았다.")
    ...
    image(fig, A / "hook_v2_overlay.png", (X[2], 0.45, W, H),
          ...
          "어두워서 자연히 분리된다. 노란 부분이 고리 꼭대기, 십자가 추정한 고리 위치다. 네 번 촬영에서 이 위치의 흩어짐(RMS)은 3.85 mm 다.")
